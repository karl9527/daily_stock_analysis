...
class TushareFetcher(BaseFetcher):
    ...
    def _init_api(self) -> None:
        ...
        支持通过 TUSHARE_API_URL 环境变量配置自定义 API 地址（如第三方代理）。
        """
        config = get_config()
        ...
        try:
            ...
            # 如果配置了自定义 API 地址，直接设置到 SDK 内部属性
            custom_url = config.tushare_api_url
            if custom_url:
                self._api._DataApi__token = config.tushare_token
                self._api._DataApi__http_url = custom_url
                # 同时 patch query 方法，确保请求发到自定义地址
                self._patch_api_endpoint(config.tushare_token, custom_url)
            else:
                # 未配置自定义地址时，patch 到官方 api.tushare.pro
    ...
    def _patch_api_endpoint(self, token: str, api_url: Optional[str] = None) -> None:
        """
        ...
        POST directly to the target URL (root URL, no path suffix).

        Args:
            token: Tushare API token
            api_url: 自定义 API 地址，为 None 时使用官方 api.tushare.pro
        """
        import types

        TUSHARE_API_URL = api_url or "http://api.tushare.pro"
        _token = token
